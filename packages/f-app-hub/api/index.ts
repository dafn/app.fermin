...
export const datastore = {
  addCard: ({ title, href = '#', background = 'white', textColor = 'black', image }: CardType, onSuccess?: onSuccessType, onError?: onErrorType ) => {
    graphql.request(`mutation { addCard(title: "${title}", href: "${href}", background: "${background}", textColor: "${textColor}", image: "${image}") }`)

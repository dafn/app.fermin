...
const Element = ({ index, value, active }: ElementProps) => {
  ...
  return (
    <section
      className={`ListElement_main_container ${active ? "active" : ""}`}
      onClick={() => actions.setActiveKey(index)}
    >
      <article className="ListElement_text_container">
        <h1 className="ListElement_title_container">
          <ReactQuill
            className="ListElement_editor"
            value={title && title[0] !== "br>" ? title[0] : "Untitled Note"}
            readOnly
            modules={modules}
          />
        </h1>
